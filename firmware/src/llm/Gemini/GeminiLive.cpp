...
void GeminiLive::load_role(){
  ...
  // --- 1. SDカードのYAMLからのプロンプトを安全にチェック ---
  String sdPrompt = "";
  if (robot != nullptr) {
    sdPrompt = robot->m_config.getExConfig().llm.system_prompt;
  } else {
    Serial.println("Warning: 'robot' pointer is null in load_role()");
  }
